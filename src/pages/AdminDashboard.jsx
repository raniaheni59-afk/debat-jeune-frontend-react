...
/* ═══════════════════════════════════════
   CALENDAR NOTES WIDGET — visible only in Admin dashboard
   Reads notes saved by CalendarPage from localStorage.
   Admin can add/delete notes freely.
═══════════════════════════════════════ */
const CAT_COLORS_WIDGET = {
...
function CalendarNotesWidget({ onOpenCalendar }) {
  ...
  const [form,      setForm]      = useState({ title:"", text:"", category:"Evenement", time:"" });
  const [activeDay, setActiveDay] = useState(null);

  // Collect all notes flat, sorted by date
  const allNotes = Object.entries(notes)
    .flatMap(([date, arr]) => arr.map((n, i) => ({ ...n, date, idx: i })))
    .sort((a, b) => a.date.localeCompare(b.date));

  const saveNote = () => {
    if (!form.title.trim() || !selDate) return;
    setNotes(prev => {
      const updated = { ...prev, [selDate]: [...(prev[selDate] || []), { ...form }] };
      localStorage.setItem("swafy_calendar_notes", JSON.stringify(updated));
      return updated;
    });
    setAddModal(false);
    setForm({ title:"", text:"", category:"Evenement", time:"" });
  };

  const deleteNote = (date, idx) => {
    setNotes(prev => {
      const updated = { ...prev, [date]: (prev[date]||[]).filter((_,i) => i !== idx) };
  ...
  const grouped = Object.entries(
    allNotes.reduce((acc, n) => { (acc[n.date] = acc[n.date] || []).push(n); return acc; }, {})
  ).sort(([a],[b]) => a.localeCompare(b));

  return (
    <div style={{ background:"rgba(255,255,255,.93)", backdropFilter:"blur(10px)", borderRadius:18, border:"1px solid rgba(255,255,255,.5)", boxShadow:"0 4px 24px rgba(100,70,180,.1)", padding:"24px 28px", marginTop:24 }}>
      {/* Header */}
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:18, flexWrap:"wrap", gap:10 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <div style={{ width:38, height:38, borderRadius:12, background:"linear-gradient(135deg,#0ABFAA,#089E8C)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18 }}>📅</div>
          <div>
            <h3 style={{ fontSize:16, fontWeight:700, color:"#2d2555", fontFamily:"Poppins,sans-serif", margin:0 }}>Notes du Calendrier</h3>
            <p style={{ fontSize:11, color:"#999", margin:0 }}>{allNotes.length} note{allNotes.length !== 1 ? "s" : ""} enregistrée{allNotes.length !== 1 ? "s" : ""}</p>
          </div>
        </div>
        <div style={{ display:"flex", gap:8 }}>
          <button
            onClick={() => setAddModal(true)}
            style={{ padding:"8px 18px", borderRadius:10, border:"none", background:"linear-gradient(135deg,#0ABFAA,#089E8C)", color:"#fff", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"Poppins,sans-serif", boxShadow:"0 2px 8px rgba(10,191,170,.3)", display:"flex", alignItems:"center", gap:6 }}>
            ＋ Ajouter une note
          </button>
          {onOpenCalendar && (
            <button
              onClick={onOpenCalendar}
              style={{ padding:"8px 18px", borderRadius:10, border:"1.5px solid #e0dce8", background:"#faf8ff", color:"#5a3fa0", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"Poppins,sans-serif" }}>
              📅 Voir le calendrier
            </button>
          )}
        </div>
      ...
      {allNotes.length === 0 ? (
        <div style={{ textAlign:"center", padding:"40px 0", color:"#bbb" }}>
          ...
          <p style={{ fontSize:13, fontWeight:600, color:"#888" }}>Aucune note dans le calendrier</p>
          <p style={{ fontSize:12, color:"#bbb", marginTop:4 }}>Ajoutez des notes depuis le calendrier ou via ce widget.</p>
        </div>
      ) : (
          ...
          {grouped.map(([date, dayNotes]) => (
            <div key={date}>
              <div
                ...
                <div style={{ width:3, height:14, borderRadius:2, background:"#0ABFAA" }}/>
                <span style={{ fontSize:12, fontWeight:700, color:"#5a3fa0", fontFamily:"Poppins,sans-serif" }}>
                  {formatDate(date)}
                </span>
                <span style={{ fontSize:10, background:"#E6F9F7", color:"#065E56", padding:"2px 7px", borderRadius:10, fontWeight:700 }}>
                  {dayNotes.length}
                </span>
                <span style={{ marginLeft:"auto", fontSize:11, color:"#bbb" }}>{activeDay === date ? "▲" : "▼"}</span>
              </div>
              {(activeDay === null || activeDay === date) && (
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(250px,1fr))", gap:10 }}>
                  {dayNotes.map((n) => {
                    const col = CAT_COLORS_WIDGET[n.category] || CAT_COLORS_WIDGET.Personnel;
                    return (
                      <div key={`${n.date}-${n.idx}`}
                        style={{ background:col.light, borderRadius:12, padding:"12px 14px", border:`1px solid ${col.bg}30`, position:"relative", transition:"box-shadow .18s" }}
                        onMouseEnter={e => e.currentTarget.style.boxShadow=`0 4px 16px ${col.bg}30`}
                        onMouseLeave={e => e.currentTarget.style.boxShadow="none"}>
                        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:8 }}>
                          <div style={{ flex:1 }}>
                            <span style={{ display:"inline-block", background:col.bg, color:"#fff", fontSize:9, fontWeight:800, padding:"2px 7px", borderRadius:6, marginBottom:5, textTransform:"uppercase", letterSpacing:.5 }}>
                              {n.category === "Enquete" ? "Enquête" : n.category === "Evenement" ? "Événement" : n.category}
                            </span>
                            <p style={{ fontSize:13, fontWeight:700, color:col.text, margin:"0 0 3px", fontFamily:"Poppins,sans-serif" }}>{n.title}</p>
                            {n.time && <p style={{ fontSize:11, color:"#888", margin:"0 0 4px" }}>🕐 {n.time}</p>}
                            {n.text && <p style={{ fontSize:11, color:"#666", lineHeight:1.45, margin:0 }}>{n.text}</p>}
                          </div>
                          <button
                            onClick={() => deleteNote(n.date, n.idx)}
                            style={{ background:"rgba(239,68,68,.1)", border:"none", color:"#EF4444", width:26, height:26, borderRadius:7, cursor:"pointer", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12 }}>
                            ✕
                          </button>
                        </div>
                      </div>
      ...
      {/* Add Note Modal */}
      {addModal && (
        <div style={{ position:"fixed", inset:0, background:"rgba(15,23,42,.45)", backdropFilter:"blur(6px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:9999 }}>
          <div style={{ background:"#fff", borderRadius:16, width:440, maxWidth:"95vw", boxShadow:"0 24px 64px rgba(0,0,0,.18)", overflow:"hidden", animation:"calPop .22s ease" }}>
            <style>{`@keyframes calPop{from{transform:scale(.94);opacity:0}to{transform:scale(1);opacity:1}}`}</style>
            <div style={{ padding:"20px 24px", borderBottom:"1px solid #E8EDF5", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <span style={{ fontSize:15, fontWeight:700, color:"#1A2340", fontFamily:"Poppins,sans-serif" }}>Nouvelle note calendrier</span>
              <button onClick={() => setAddModal(false)} style={{ background:"none", border:"none", cursor:"pointer", fontSize:17, color:"#94A3B8", width:30, height:30, borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center" }}>✕</button>
            </div>
            <div style={{ padding:"20px 24px", display:"flex", flexDirection:"column", gap:14 }}>
              <div>
                <label style={{ fontSize:11, color:"#94A3B8", display:"block", marginBottom:4, fontWeight:700, textTransform:"uppercase", letterSpacing:.5 }}>Date</label>
                <input type="date" value={selDate} onChange={e => setSelDate(e.target.value)}
                  style={{ width:"100%", padding:"9px 12px", border:"1.5px solid #E8EDF5", borderRadius:8, fontSize:13, outline:"none", fontFamily:"Poppins,sans-serif", color:"#4A5568" }} />
              </div>
              <input value={form.title} onChange={e => setForm(p=>({...p,title:e.target.value}))}
                placeholder="Titre"
                style={{ border:"none", borderBottom:"1.5px solid #E8EDF5", padding:"6px 0", fontSize:17, outline:"none", color:"#1A2340", fontFamily:"Poppins,sans-serif", fontWeight:600, background:"transparent" }} />
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                <div>
                  <label style={{ fontSize:11, color:"#94A3B8", display:"block", marginBottom:4, fontWeight:700, textTransform:"uppercase", letterSpacing:.5 }}>Heure</label>
                  <input type="time" value={form.time} onChange={e => setForm(p=>({...p,time:e.target.value}))}
                    style={{ width:"100%", padding:"9px 12px", border:"1.5px solid #E8EDF5", borderRadius:8, fontSize:13, outline:"none", fontFamily:"Poppins,sans-serif", color:"#4A5568" }} />
                </div>
                <div>
                  <label style={{ fontSize:11, color:"#94A3B8", display:"block", marginBottom:4, fontWeight:700, textTransform:"uppercase", letterSpacing:.5 }}>Catégorie</label>
                  <select value={form.category} onChange={e => setForm(p=>({...p,category:e.target.value}))}
                    style={{ width:"100%", padding:"9px 12px", border:"1.5px solid #E8EDF5", borderRadius:8, fontSize:13, outline:"none", fontFamily:"Poppins,sans-serif", color:"#4A5568", background:"#fff" }}>
              ...
              <textarea value={form.text} onChange={e => setForm(p=>({...p,text:e.target.value}))}
                ...
                style={{ border:"1.5px solid #E8EDF5", borderRadius:8, padding:10, fontSize:12, resize:"vertical", minHeight:70, outline:"none", fontFamily:"Poppins,sans-serif" }} />
            </div>
            <div style={{ padding:"12px 24px 20px", display:"flex", justifyContent:"flex-end", gap:8 }}>
              <button onClick={() => setAddModal(false)} style={{ padding:"9px 20px", border:"none", borderRadius:8, background:"none", color:"#0ABFAA", fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"Poppins,sans-serif" }}>Annuler</button>
              <button onClick={saveNote} style={{ padding:"9px 22px", border:"none", borderRadius:8, background:"#0ABFAA", color:"#fff", fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"Poppins,sans-serif", boxShadow:"0 2px 8px rgba(10,191,170,.3)" }}>Enregistrer</button>
            </div>
          </div>
        </div>
      )}
        ...
        >
          ...
          <div style={S.actions}>
            ...
            <button style={{ ...S.actBtn, ...S.actAdd }} onClick={() => openAddData(chart)}>
              ...
              <CalendarNotesWidget onOpenCalendar={() => goTo("calendrier")} />
