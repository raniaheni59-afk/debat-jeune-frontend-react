...
function Av({user,size=42}) {
  ...
  return (
    <div style={{width:size,height:size,borderRadius:"50%",flexShrink:0,background:avatarCol(user?.id_user),
      ...
      color:"#fff",fontWeight:700,fontSize:Math.round(size*.35),userSelect:"none"}}>
...
export default function JeuneContact() {
  ...
  const myId = Number(JSON.parse(localStorage.getItem("user")||"{}").id_user||0);
    ...
    sock.on("newMessage", (m) => {
      ...
      if (cur && cur!=="group" && Number(m.conversation_id)===Number(cur.id)) {
        setMsgs(p => {
          ...
          // Mon propre message (déjà affiché via optimiste) → ignorer socket
          if (Number(m.sender_id)===myId) return p;
          // Message de l'autre → ajouter direct
          return [...p.filter(x=>!x._temp), ...p.filter(x=>x._temp), m];
        });
      }
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(async () => {
      try {
        const lq=q.toLowerCase();
        const filtered=admins.filter(u=>`${u.prenom_user||""} ${u.nom_user||""}`.toLowerCase().includes(lq));
    ...
    <div className="admin-contact">
      ...
      <aside className="contacts-panel">
        ...
        <div className="chat-list">
          ...
          {/* Admins sans conversation — bouton Contacter */}
            ...
            {loadingMsgs?(
              ...
              return (
                <div key={m.id} style={{alignSelf:isMe?"flex-end":"flex-start",maxWidth:"72%",display:"flex",flexDirection:"column",gap:3,animation:"fadeUp .18s ease"}}>
                  ...
                  <div style={{background:isMe?"linear-gradient(135deg,#6d56c1,#4f3fa0)":"#fff",color:isMe?"#fff":"#1a1a2e",
                    padding:"10px 14px",borderRadius:isMe?"18px 18px 4px 18px":"18px 18px 18px 4px",
                    fontSize:13.5,lineHeight:1.55,boxShadow:isMe?"0 4px 14px rgba(79,63,160,0.3)":"0 1px 4px rgba(0,0,0,0.07)",
                    border:isMe?"none":"1px solid #ede9ff",wordBreak:"break-word",opacity:m._temp?0.6:1}}>
