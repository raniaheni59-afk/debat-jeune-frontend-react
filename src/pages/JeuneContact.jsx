...
function Av({ user, size=42 }) {
  ...
  return (
    <div style={{width:size,height:size,borderRadius:"50%",flexShrink:0,background:avatarCol(user?.id_user),display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:700,fontSize:Math.round(size*.35),userSelect:"none"}}>
...
function AdminAv({ size=42 }) {
  return (
    <div style={{width:size,height:size,borderRadius:"50%",flexShrink:0,background:"linear-gradient(135deg,#6d56c1,#4f3fa0)",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:800,fontSize:Math.round(size*.42),userSelect:"none"}}>S</div>
  );
}
...
function FileBubble({ msg, isMe }) {
  ...
  return (
    <a href={url} target="_blank" rel="noreferrer" style={{display:"inline-flex",alignItems:"center",gap:8,marginTop:msg.text?6:0,padding:"9px 14px",borderRadius:10,fontSize:13,fontWeight:600,textDecoration:"none",background:isMe?"rgba(255,255,255,0.18)":"#f0ecff",color:isMe?"#fff":"#6d56c1",border:isMe?"1px solid rgba(255,255,255,0.25)":"1px solid #ddd6fe"}}>
...
export default function JeuneContact() {
  ...
  const openConv = async (targetUser) => {
    ...
    try {
      ...
      setSel(conv);
      // ✅ Dedup — évite la double conversation dans la sidebar
      setConvs(p => {
  ...
  const send = async () => {
    ...
    try {
      ...
      if (sel==="group") {
        if (file) {
          const fd=new FormData(); fd.append("file",file); if(msgText) fd.append("text",msgText);
        ...
        if (file) {
          const fd=new FormData(); fd.append("file",file); fd.append("conversationId",String(sel.id)); if(msgText) fd.append("text",msgText);
      ...
      if(msgText) setText(msgText); if(file) setFilePrev(file);
  ...
  // Admins sans conversation existante — pour le bouton "Contacter"
        ...
        {!sel ? (
            ...
            {loadingMsgs ? (
              ...
              return (
                <div key={m.id} style={{alignSelf:isMe?"flex-end":"flex-start",maxWidth:"72%",display:"flex",flexDirection:"column",gap:3,animation:"fadeUp .18s ease"}}>
                  ...
                  <div style={{background:isMe?"linear-gradient(135deg,#6d56c1,#4f3fa0)":"#fff",color:isMe?"#fff":"#1a1a2e",padding:"10px 14px",borderRadius:isMe?"18px 18px 4px 18px":"18px 18px 18px 4px",fontSize:13.5,lineHeight:1.55,boxShadow:isMe?"0 4px 14px rgba(79,63,160,0.3)":"0 1px 4px rgba(0,0,0,0.07)",border:isMe?"none":"1px solid #ede9ff",wordBreak:"break-word",opacity:m._temp?0.65:1}}>
          ...
          {filePrev && (
            <div className="file-preview-bar">
              <span style={{fontSize:16}}>{filePrev.type.startsWith("image/")?"🖼️":filePrev.type.startsWith("video/")?"🎬":"📄"}</span>
              <span style={{flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{filePrev.name}</span>
          ...
          <div style={{padding:"10px 14px",borderTop:"1px solid #ede9ff",display:"flex",alignItems:"flex-end",gap:8,background:"#fff",flexShrink:0}}>
            ...
            <button onClick={send} disabled={!canSend}
              style={{width:42,height:42,borderRadius:12,border:"none",flexShrink:0,background:canSend?"linear-gradient(135deg,#6d56c1,#4f3fa0)":"#e8e3ff",color:canSend?"#fff":"#c4bde8",cursor:canSend?"pointer":"default",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:canSend?"0 4px 14px rgba(79,63,160,0.38)":"none",transition:"all .2s"}}>
              {sending
