...
const JeuneLayout = () => {
  ...
  // ✅ sidebar fermée par défaut — overlay n'apparaît JAMAIS sur desktop
  ...
  /* ── Deep link vers une publication ── */
  useEffect(() => {
    const pubId = new URLSearchParams(window.location.search).get("publication");
    if (pubId) {
  ...
  const handleNav = (item, idx) => {
    ...
    // ✅ ferme sidebar seulement sur mobile
  ...
  return (
    <div className="jl-root">
      ...
      {/* ✅ Overlay UNIQUEMENT mobile ET quand sidebar ouverte */}
      {mobileSidebarOpen && isMobile() && (
        <div className="jl-overlay" onClick={() => setMobileSidebarOpen(false)} />
      )}

      {/* ── SIDEBAR ── */}
      <aside className={`jl-sidebar ${mobileSidebarOpen ? "open" : ""}`}>
      ...
      <aside className="jl-right">
        <div className="jl-profile-card" onClick={() => navigate("/profile")}>
          <img
            src={getAvatar(user?.photo_user, user?.sexe)}
            alt="avatar" className="jl-profile-avatar"
            onError={e => e.target.src = "https://randomuser.me/api/portraits/men/44.jpg"}
          />
