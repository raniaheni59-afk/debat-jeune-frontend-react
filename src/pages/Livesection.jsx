...
export default function LiveSection() {
  ...
  useEffect(() => {
    ...
    /* ✅ FIX CRITIQUE: live-started → mettre à jour le live immédiatement */
    sock.on("live-started", ({ roomCode, viewerLink, hostName, title, description, thematique }) => {
      const newLive = {
        roomCode,
  ...
  const joinLive = () => {
    ...
    if (roomCode && vt) {
      ...
      alert("Lien expiré — attendez que l'admin envoie un nouveau lien.");
    }
  };
  ...
  return (
    <div style={{ fontFamily: "'Inter',sans-serif", position: "relative" }}>
      ...
      {/* ALERTE LIVE DÉMARRÉ */}
      {alert && (alertData || live) && (
        <div style={S.alert}>
          <span style={S.alertDot} />
          🎙️ <strong>{(alertData || live).hostName}</strong> a démarré un live — rejoignez maintenant !
          {(alertData || live).title !== "Live en cours" && <em style={{ opacity: .85, marginLeft: 4 }}>"{(alertData || live).title}"</em>}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={S.preview}>
            ...
            <div style={{ textAlign: "center", zIndex: 1 }}>
              ...
              {live.thematique && <p style={{ color: "rgba(255,255,255,.5)", fontSize: 11, marginTop: 4 }}>🎯 {live.thematique}</p>}
            </div>
          ...
          {live.streamLink && (
            <div style={S.linkBox}>
              ...
              <button onClick={copyLink} style={S.copyBtn} title="Copier le lien">
                {copied ? "✅" : "📋"}
              </button>
            </div>
          )}
