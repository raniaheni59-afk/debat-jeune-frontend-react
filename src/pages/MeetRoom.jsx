...
.emoji-btn:hover{transform:scale(1.28);}
...
function useSubtitles(enabled, lang) {
  ...
  useEffect(() => {
    ...
    r.onresult = e => {
      ...
      if (finals.length) setFull(p => [...p, ...finals]);
    };
    r.onerror = () => {};
...
function VideoTile({ stream, muted=false, name="?", role="guest", videoOff=false, hand=false, isLocal=false, localRef, screenSharing=false }) {
  ...
  useEffect(() => {
    ...
    if (stream) {
      ...
      // local video: srcObject set from parent via ref, nothing to do here
    }
  }, [stream]);
  ...
  return (
    <div style={{
      ...
      boxShadow: hand ? "0 0 0 3px #fbbc04" : isHost ? "0 0 0 2px rgba(26,115,232,.45)" : "none",
      transition: "box-shadow .3s"
    }}>
      {/* Video — rendered always, hidden when off */}
      <video ref={vRef} autoPlay playsInline muted={muted}
        style={{ width:"100%", height:"100%", objectFit:"cover", background:"#000",
          display:(videoOff || (!stream && !isLocal)) ? "none" : "block" }} />
      ...
      <div style={{ position:"absolute",bottom:8,left:8,display:"flex",gap:5,alignItems:"center",flexWrap:"wrap" }}>
        {isHost && <span style={{ background:"rgba(26,115,232,.9)",color:"#fff",fontSize:9,fontWeight:800,padding:"2px 7px",borderRadius:20 }}>👑 HOST</span>}
        {screenSharing && <span style={{ background:"rgba(52,168,83,.9)",color:"#fff",fontSize:9,padding:"2px 7px",borderRadius:8 }}>🖥️ Partage</span>}
        ...
        {videoOff && <span style={{ background:"rgba(234,67,53,.8)",color:"#fff",fontSize:9,padding:"2px 6px",borderRadius:6 }}>📷 Off</span>}
      </div>
...
export default function MeetRoom() {
  ...
  // ── Role detection: at= → host, vt= → guest ──────────
  ...
  const [videoOn,       setVideoOn]      = useState(true);   // host only
  ...
  const localVid   = useRef(null);   // admin camera video element
  ...
  const { text: subtitle, full: fullTranscript } = useSubtitles(subs, subLang);

  // Viewer link: stored when admin creates the live via NewLive.jsx
  ...
  const createPeer = useCallback((tid) => {
    ...
    pc.ontrack = e => {
      ...
      setPeers(prev => {
        ...
        if (ex) return prev.map(p=>p.id===tid?{...p,stream}:p);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        if (myRole === "host") {
          // Admin: get camera + mic
          const st = await navigator.mediaDevices.getUserMedia({ video:true, audio:true });
          if (!alive) { st.getTracks().forEach(t=>t.stop()); return; }
          localStr.current = st;
          // ✅ Set srcObject after state updates
          setTimeout(() => {
            if (localVid.current) localVid.current.srcObject = st;
          }, 100);
        } else {
          // Jeune: mic only, no camera
          const st = await navigator.mediaDevices.getUserMedia({ video:false, audio:true });
          if (!alive) { st.getTracks().forEach(t=>t.stop()); return; }
          localStr.current = st;
          // Mute jeune by default — they speak when they want
          st.getAudioTracks().forEach(t => { t.enabled = false; });
          setAudioOn(false);
        }
      } catch {
        setAudioOn(false);
        if (myRole === "host") setVideoOn(false);
      }
      ...
      sock.on("connect", () => {
        sock.emit("join-room", { roomCode, userName:myName, role:myRole, accessToken:token }, ack => {
          if (ack && !ack.ok) { setErrorMsg(ack.message||"Accès refusé"); setStatus("error"); }
        });
      });
      ...
      // ✅ Messages arrive to EVERYONE in the room
  ...
  const copyLink = async () => {
    const lnk = viewerLink || window.location.href.replace(/\?at=.*/,"?vt=LINK_MISSING");
  ...
  const generateConclusion = async () => {
    ...
    try {
      ...
      const prompt = `Tu es expert en résumé de réunions.\n\nRéunion "${roomCode}":\n\nTRANSCRIPTION: ${fullTranscript.join(" ")||"(aucune)"}\n\nCHAT:\n${chatContent||"(aucun)"}\n\nSTATS: ${ptcps.length} participants, durée: ${fmtTime(duration)}, date: ${new Date().toLocaleDateString("fr-FR",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}\n\nGénère une conclusion structurée avec: 📋 Résumé, 💡 Idées clés, ✅ Consensus, 🎯 Actions, 📊 Stats. Sois concis et professionnel.`;
      const res = await fetch(ANTHROPIC_URL, { method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{role:"user",content:prompt}]})});
      const d = await res.json();
      setAiResult(d.content?.[0]?.text||"Erreur.");
  ...
  const handleTranslate = async (id,text) => {
    ...
    try { const t=await aiTranslate(text,transLang); setTranslations(p=>({...p,[id]:t})); } catch {}
    setTranslating(p=>({...p,[id]:false}));
  };

  // ── Grid ──────────────────────────────────────────────
  // Guest sees ONLY the host tile + their own avatar
  // Host sees everyone
  const hostPeer   = peers.find(p => (ptcps.find(x=>x.socketId===p.id)?.role||roleMap.current[p.id])==="host");
  const tilesToShow = myRole==="host" ? peers : (hostPeer ? [hostPeer] : []);
  ...
  return (
    <div style={{minHeight:"100vh",height:"100vh",background:"#202124",display:"flex",flexDirection:"column",fontFamily:"'Google Sans',system-ui,sans-serif",color:"#fff",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          ...
          {myRole==="host" && (
            <button onClick={()=>setShowLink(o=>!o)} style={{background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",color:"#e8eaed",padding:"6px 14px",borderRadius:8,cursor:"pointer",fontSize:12}}>
              🔗 {copied?"Copié !":"Inviter les jeunes"}
            </button>
          )}
      ...
      {/* SHARE LINK BAR */}
      {showLink && myRole==="host" && (
        <div style={{background:"#2d2f31",borderBottom:"1px solid rgba(255,255,255,.06)",padding:"10px 18px",flexShrink:0}}>
          <div style={{color:"#9aa0a6",fontSize:11,marginBottom:6}}>🔗 Copiez ce lien et envoyez-le aux jeunes inscrits</div>
        ...
        <div style={{background:"#2d2f31",borderBottom:"1px solid rgba(255,255,255,.06)",padding:"10px 18px",flexShrink:0}}>
          <div style={{display:"flex",gap:20,alignItems:"center",flexWrap:"wrap"}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{color:"#9aa0a6",fontSize:12}}>🌍 Traduction:</span>
            ...
            {subs && (
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{color:"#9aa0a6",fontSize:12}}>🎤 Sous-titres:</span>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden",gap:8,padding:8,minHeight:0}}>
        ...
        <div style={{flex:1,display:"grid",gap:8,alignContent:"center",overflow:"hidden",minWidth:0,gridTemplateColumns:`repeat(${cols},1fr)`}}>
          ...
          {/* LOCAL TILE */}
          {myRole==="host" ? (
            // Admin: show their own camera
            <VideoTile localRef={localVid} isLocal muted name={myName} role="host" videoOff={!videoOn} />
          ) : (
            // Jeune: show avatar + mic status (no camera)
            <div style={{position:"relative",background:"#1a1a1a",borderRadius:12,overflow:"hidden",aspectRatio:"16/9",minHeight:120}}>
              <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:10,background:"radial-gradient(circle at 35% 30%,#2a2a2a,#111)"}}>
                ...
                <span style={{color:"#9aa0a6",fontSize:11,background:"rgba(0,0,0,.4)",padding:"4px 10px",borderRadius:20}}>
                  {audioOn?"🎤 Micro actif":"🔇 Micro coupé"}
                </span>
              </div>
          ...
          {/* REMOTE TILES */}
          {tilesToShow.map(p => (
            <VideoTile key={p.id} stream={p.stream}
        ...
        {chatOpen && (
          <div style={{width:320,background:"#2d2f31",borderRadius:12,border:"1px solid rgba(255,255,255,.06)",display:"flex",flexDirection:"column",overflow:"hidden",flexShrink:0,animation:"slideIn .25s ease"}}>
            ...
            <div style={{flex:1,overflowY:"auto",padding:10,display:"flex",flexDirection:"column",gap:8}}>
              {msgs.length===0 && <div style={{color:"#5f6368",textAlign:"center",padding:"32px 16px",fontSize:13}}><div style={{fontSize:36,marginBottom:8}}>💬</div>Pas encore de messages…</div>}
              {msgs.map(m=>{
                const isMe = m.user===myName;
                return (
                  <div key={m.id} style={{alignSelf:isMe?"flex-end":"flex-start",maxWidth:"85%",animation:"fadeIn .2s ease"}}>
                    {!isMe && (
                    ...
                    <div style={{fontSize:10,color:"#5f6368",marginTop:2,display:"flex",gap:8}}>
                      <span>{m.time}</span>
                      {!isMe&&(
            ...
            {/* Input — TOUS les utilisateurs peuvent écrire */}
          ...
          <div style={{width:300,background:"#2d2f31",borderRadius:12,border:"1px solid rgba(255,255,255,.06)",display:"flex",flexDirection:"column",overflow:"hidden",flexShrink:0,animation:"slideIn .25s ease"}}>
            ...
            {myRole==="host"&&(
              <div style={{padding:10,borderTop:"1px solid rgba(255,255,255,.06)"}}>
                <button onClick={muteAll} style={{background:"rgba(234,67,53,.12)",border:"1px solid rgba(234,67,53,.25)",borderRadius:8,color:"#ea4335",padding:"8px 12px",cursor:"pointer",fontWeight:600,fontSize:12,width:"100%"}}>
            ...
            <div style={{flex:1,overflowY:"auto",padding:14}}>
              {aiLoading ? (
                ...
                <div>
                  ...
                  <button onClick={()=>{navigator.clipboard.writeText(aiResult);showToast("Copié !","#34a853","📋");}}
                ...
                <div style={{textAlign:"center",padding:"20px 10px"}}>
                  ...
                  <p style={{color:"#9aa0a6",fontSize:13,marginBottom:20,lineHeight:1.7}}>L'IA analysera la transcription et le chat pour générer une conclusion archivable.</p>
                  <button onClick={generateConclusion}
      ...
      <div style={{background:"#2d2f31",padding:"10px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,flexShrink:0,flexWrap:"wrap",borderTop:"1px solid rgba(255,255,255,.05)"}}>
        ...
        <div style={{display:"flex",gap:6}}>
          ...
          {myRole==="host"&&<Btn icon={videoOn?"📷":"🚫"} label={videoOn?"Caméra":"Off"} onClick={toggleVideo} active={videoOn}/>}
        </div>
        {/* Center */}
        <div style={{display:"flex",gap:6}}>
          {myRole==="host"&&<Btn icon="🖥️" label={screen?"Arrêter":"Partager"} onClick={toggleScreen} active={!screen}/>}
          ...
          {myRole==="host"&&<Btn icon="👥" label="Membres" onClick={()=>{setPanelOpen(o=>!o);setChatOpen(false);setAiOpen(false);}} active/>}
        </div>
        {/* Right */}
        <div style={{display:"flex",gap:6}}>
          <button className="ctrl-btn" onClick={()=>setSubs(o=>!o)} title="Sous-titres"
            style={{border:"none",borderRadius:12,padding:"10px 14px",color:"#fff",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,minWidth:60,transition:"all .15s",background:subs?"#1a73e8":"rgba(255,255,255,.1)"}}>
          ...
          {myRole==="host"&&<Btn icon="✨" label="Conclusion" onClick={()=>{setAiOpen(o=>!o);setChatOpen(false);setPanelOpen(false);}} active/>}
