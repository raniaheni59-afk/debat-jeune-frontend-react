...
export default function MeetRoom() {
  ...
  const createPeer = useCallback((sid) => {
    ...
    // Si partage d'écran actif → utiliser le screen track pour la vidéo
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // L'admin utilise un délai plus long pour laisser le temps au stream de s'initialiser
      sock.on("all-users", async users => {
        for (const u of users) {
          ...
          // Admin envoie offre à tous, guest attend l'offre de l'admin
          if (myRole === "host") {
            setTimeout(() => createOfferForPeer(u.socketId), 600);
          }
        }
      });
      ...
      sock.on("user-joined", ({ socketId, userName:n, role, email }) => {
        ...
        // ✅ FIX PRINCIPAL: l'admin envoie toujours l'offre au nouveau participant
        // Le délai permet au guest de finir son join-room et créer son peer
        if (myRole === "host") {
          setTimeout(() => createOfferForPeer(socketId), 500);
        }
      });

      // Quand l'admin rejoint: mise à jour des maps — l'admin enverra l'offre WebRTC
      sock.on("host-joined", ({ socketId, userName:n }) => {
        nameMap.current[socketId] = n;
        roleMap.current[socketId] = "host";
        showToast(`👑 ${n} (Admin) a rejoint`, "#1a73e8");
        // NE PAS créer de peer ici — l'admin fait createOfferForPeer via user-joined
        // Si on crée un peer sans stream local prêt, la négociation WebRTC échoue
      });
    ...
    if (screenOn) {
      ...
      try {
        ...
        const replacePromises = Object.values(pcMap.current).map(pc => {
          ...
          // Si pas de sender vidéo, ajouter le track directement
  ...
  const adminKick     = sid => { if(myRole!=="host") return; setKickTarget({ sid, name: nameMap.current[sid]||"ce participant" }); };
  const adminBlock    = sid => { if(myRole!=="host") return; setBlockTarget({ sid, name: nameMap.current[sid]||"ce participant" }); };
  const confirmKick   = () => { if(!kickTarget) return; emit("admin-kick",{roomCode,targetSocketId:kickTarget.sid}); setKickTarget(null); showToast("Retiré","#ea4335","🚪"); };
  const confirmBlock  = () => { if(!blockTarget) return; emit("admin-block",{roomCode,targetSocketId:blockTarget.sid,targetEmail:emailMap.current[blockTarget.sid]||""}); setBlockTarget(null); showToast("Bloqué 🚫","#ea4335"); };
  ...
  return (
    <div style={{ height:"100vh",display:"flex",flexDirection:"column",background:"#202124",fontFamily:"'Google Sans',system-ui,sans-serif",color:"#fff",overflow:"hidden" }}>
      ...
      {/* MODALS */}
      {kickTarget  && <Modal emoji="🚪" title={`Retirer ${kickTarget.name} ?`} desc="Il sera exclu de la session (peut revenir avec le lien)." onCancel={()=>setKickTarget(null)} onConfirm={confirmKick} confirmLabel="Retirer" />}
      {blockTarget && <Modal emoji="🚫" title={`Bloquer ${blockTarget.name} définitivement ?`} desc="Son compte sera bloqué et il ne pourra plus rejoindre aucun live." onCancel={()=>setBlockTarget(null)} onConfirm={confirmBlock} confirmLabel="🚫 Bloquer" />}
