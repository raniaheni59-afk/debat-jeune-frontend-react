...
function Tile({ stream, muted=false, name="?", role="guest", camOff=false,
  ...
  return (
    <div style={{ position:"relative",background:"#111",borderRadius:14,overflow:"hidden",aspectRatio:"16/9",minHeight:110,
      boxShadow: hand?"0 0 0 3px #fbbc04,0 4px 20px rgba(251,188,4,.2)":host?"0 0 0 2px rgba(26,115,232,.45)":"0 2px 12px rgba(0,0,0,.4)" }}>
      <video ref={vRef} autoPlay playsInline muted={muted}
        style={{ width:"100%",height:"100%",objectFit:"cover",display:showV?"block":"none" }} />
      {!showV && (
        <div style={{ position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"radial-gradient(circle at 35% 30%,#1c1c2e,#0d0d1a)" }}>
          <div style={{ width:72,height:72,borderRadius:"50%",fontSize:26,fontWeight:800,color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",
            background:host?"linear-gradient(135deg,#1a73e8,#0d47a1)":"linear-gradient(135deg,#34a853,#1a6e38)",boxShadow:"0 6px 24px rgba(0,0,0,.5)" }}>
            {init}
          </div>
        </div>
      )}
      <div style={{ position:"absolute",bottom:8,left:8,display:"flex",gap:5,alignItems:"center",flexWrap:"wrap" }}>
        {host && <span style={{ background:"rgba(26,115,232,.9)",color:"#fff",fontSize:9,fontWeight:800,padding:"2px 8px",borderRadius:20 }}>👑 ADMIN</span>}
...
function Modal({ title, emoji, desc, onCancel, onConfirm, confirmLabel, confirmColor="#ea4335", children }) {
  return (
...
export default function MeetRoom() {
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND_URL}/api/lives`, { headers:{ Authorization:`Bearer ${localStorage.getItem("token")||""}` } })
  ...
  const assignLocalVideo = useCallback((stream) => {
    ...
    const try_ = () => {
      if (localVid.current) { localVid.current.srcObject=stream; localVid.current.play().catch(()=>{}); return true; }
      return false;
    };
    if (!try_()) {
  ...
  const createPeer = useCallback((sid) => {
    ...
    pc.onicecandidate = e => { if(e.candidate) sockRef.current?.emit("ice-candidate",{ target:sid,candidate:e.candidate }); };
    ...
    if (s) s.getTracks().forEach(t=>{ try { pc.addTrack(t,s); } catch {} });
    return pc;
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (myRole==="host") {
        let s = null;
        try { s = await navigator.mediaDevices.getUserMedia({ video:{ width:{ideal:1280},height:{ideal:720},facingMode:"user",frameRate:{ideal:30} }, audio:{ echoCancellation:true,noiseSuppression:true,autoGainControl:true } }); }
        catch {
          try { s = await navigator.mediaDevices.getUserMedia({ video:true,audio:true }); }
          catch {
            try { s = await navigator.mediaDevices.getUserMedia({ video:false,audio:true }); setCamOn(false); setMediaError("Caméra refusée — audio seulement"); }
            catch(err) { setPermDenied(true); setMediaError(`Permissions refusées (${err.name}). Cliquez 🔒 dans la barre → Autoriser Caméra+Micro.`); setMicOn(false); setCamOn(false); }
          }
        }
        ...
        try {
          ...
          localStr.current=s; lsRef.current=s;
        } catch { setMicOn(false); }
      }

      setStatus("ok");

      const sock = io(SOCKET_URL, { transports:["websocket"],reconnection:true,reconnectionAttempts:10,reconnectionDelay:2000 });
      sockRef.current = sock;

      sock.on("connect", () => {
        sock.emit("join-room",{ roomCode,userName:myName,role:myRole,accessToken:token,email:myEmail }, ack=>{
      ...
      sock.on("all-users", async users => {
        for (const u of users) {
          ...
          try { const offer=await pc.createOffer(); await pc.setLocalDescription(offer); sock.emit("offer",{ target:u.socketId,sdp:offer }); } catch {}
        }
      });

      sock.on("user-joined", ({ socketId,userName:n,role,email }) => {
        nameMap.current[socketId]=n; roleMap.current[socketId]=role; emailMap.current[socketId]=email||"";
      ...
      sock.on("offer", async ({ caller,sdp }) => {
        ...
        try { await pc.setRemoteDescription(new RTCSessionDescription(sdp)); const ans=await pc.createAnswer(); await pc.setLocalDescription(ans); sock.emit("answer",{ target:caller,sdp:ans }); } catch {}
      });

      sock.on("answer", async ({ responder,sdp }) => { try { await pcMap.current[responder]?.setRemoteDescription(new RTCSessionDescription(sdp)); } catch {} });
      sock.on("ice-candidate", async ({ from,candidate }) => { try { await pcMap.current[from]?.addIceCandidate(new RTCIceCandidate(candidate)); } catch {} });

      sock.on("user-left", ({ socketId }) => {
        const n=nameMap.current[socketId]; if(n) showToast(`${n} a quitté`,"#5f6368");
        pcMap.current[socketId]?.close(); delete pcMap.current[socketId];
        setPeers(prev=>prev.filter(p=>p.id!==socketId));
      });

      sock.on("user-media-toggled", ({ socketId,type,enabled }) => setPState(prev=>({ ...prev,[socketId]:{ ...prev[socketId],[type]:enabled } })));
      ...
      sock.on("force-kicked", () => { alert("Vous avez été retiré de la session."); cleanup(); navigate("/jeune"); });

      sock.on("force-blocked", ({ message }) => {
        alert(message||"Votre accès a été révoqué."); cleanup();
        localStorage.removeItem("token"); localStorage.removeItem("user");
        navigate("/login");
      });

      sock.on("live-ended", () => {
        if(myRole!=="host") { showToast("Le live est terminé.","#5f6368"); setTimeout(()=>{ cleanup(); navigate("/jeune"); },2500); }
      });
  ...
  const toggleMic = () => {
    ...
    const nxt=!micOn; tracks.forEach(t=>{ t.enabled=nxt; }); setMicOn(nxt);
  ...
  const toggleCam = () => {
    ...
    const nxt=!camOn; tracks.forEach(t=>{ t.enabled=nxt; }); setCamOn(nxt);
    emit("toggle-media",{ roomCode,type:"video",enabled:nxt });
    if(nxt&&localVid.current&&localStr.current) setTimeout(()=>{ if(localVid.current){ localVid.current.srcObject=localStr.current; localVid.current.play().catch(()=>{}); } },100);
  };
  ...
  const toggleScreen = async () => {
    ...
    if(screenOn){
      ...
      Object.values(pcMap.current).forEach(pc=>{ const s=pc.getSenders().find(s=>s.track?.kind==="video"); if(s&&cam) s.replaceTrack(cam); });
      if(localVid.current&&localStr.current){ localVid.current.srcObject=localStr.current; localVid.current.play().catch(()=>{}); }
    } else {
      try {
        const ss=await navigator.mediaDevices.getDisplayMedia({ video:true,audio:true });
        screenStr.current=ss; const trk=ss.getVideoTracks()[0];
        Object.values(pcMap.current).forEach(pc=>{ const s=pc.getSenders().find(s=>s.track?.kind==="video"); if(s) s.replaceTrack(trk); });
        trk.onended=toggleScreen; setScreenOn(true); emit("screen-share-started",{ roomCode });
  ...
  const sendMsg = () => { if(!chatInput.trim()) return; emit("send-message",{ roomCode,message:chatInput }); setChatInput(""); };
  ...
  const copyLink = async () => {
    ...
    setCopied(true); setTimeout(()=>setCopied(false),2500); showToast("✅ Lien copié !","#34a853");
  };
  ...
  const confirmKick = () => { if(!kickTarget) return; emit("admin-kick",{ roomCode,targetSocketId:kickTarget }); setKickTarget(null); showToast("Participant retiré","#ea4335","🚪"); };

  const confirmBlock = () => {
    if(!blockTarget) return;
    emit("admin-block",{ roomCode,targetSocketId:blockTarget,targetEmail:emailMap.current[blockTarget]||"" });
    setBlockTarget(null); showToast("Participant bloqué et inscription supprimée","#ea4335","🚫");
  };
  ...
  const endLive = async (withAI) => {
    setEndModal(false); let aiSummary=null;
    if(withAI){
      setAiLoading(true);
      try {
        const chat=msgsRef.current.map(m=>`${m.user}: ${m.text}`).join("\n");
        const res=await fetch(`${BACKEND_URL}/api/chatbot`,{ method:"POST", headers:{ "Content-Type":"application/json",Authorization:`Bearer ${localStorage.getItem("token")||""}` }, body:JSON.stringify({ message:`Résume ce live "${liveInfo?.title_live||roomCode}":\nParticipants: ${ptcps.length}, Durée: ${fmt(duration)}\nTranscription: ${transcript.join(" ")||"(aucune)"}\nChat:\n${chat||"(aucun)"}\n\nGénère une conclusion avec: 📋 Points principaux, 💡 Idées clés, ✅ Consensus, 🎯 Actions.` }) });
        const d=await res.json(); aiSummary=d.reply||d.message||d.content||null; setAiText(aiSummary||"");
      } catch { aiSummary=null; } setAiLoading(false);
    }
    ...
    try {
      await fetch(`${BACKEND_URL}/api/lives/archive`,{ method:"POST",headers:{ "Content-Type":"application/json",Authorization:`Bearer ${localStorage.getItem("token")||""}` },
  ...
  return (
    <div style={{ height:"100vh",display:"flex",flexDirection:"column",background:"#202124",fontFamily:"'Google Sans',system-ui,sans-serif",color:"#fff",overflow:"hidden" }}>
      ...
      {kickTarget&&<Modal title="Retirer ce participant ?" emoji="🚪" desc="Il sera exclu (peut revenir avec le lien)." onCancel={()=>setKickTarget(null)} onConfirm={confirmKick} confirmLabel="Retirer" />}
      {blockTarget&&<Modal title="Bloquer et supprimer l'inscription ?" emoji="🚫" desc="L'utilisateur sera exclu, son compte supprimé. Il ne pourra plus rejoindre." onCancel={()=>setBlockTarget(null)} onConfirm={confirmBlock} confirmLabel="🚫 Bloquer définitivement" />}

      {endModal&&(
        <Modal title="Terminer le live ?" emoji="🎙️" desc="Le live sera archivé automatiquement." onCancel={()=>setEndModal(false)} onConfirm={null} confirmLabel="">
      ...
      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 20px",background:"#2d2f31",flexShrink:0,borderBottom:"1px solid rgba(255,255,255,.06)" }}>
        ...
        <div style={{ display:"flex",gap:8,alignItems:"center" }}>
          ...
          {myRole==="host"&&<button className="cbtn" onClick={()=>setLinkOpen(o=>!o)} style={{ background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",color:"#e8eaed",padding:"6px 14px",borderRadius:8,fontSize:12,fontWeight:500 }}>🔗 Inviter</button>}
        </div>
      </div>
      ...
      {linkOpen&&myRole==="host"&&(
        <div style={{ background:"#2d2f31",borderBottom:"1px solid rgba(255,255,255,.06)",padding:"10px 18px",flexShrink:0,animation:"fadeIn .2s ease" }}>
          <p style={{ color:"#9aa0a6",fontSize:11,marginBottom:6 }}>🔗 Lien public pour les jeunes (valide 6h) :</p>
          <div style={{ display:"flex",gap:8,alignItems:"center" }}>
            <span style={{ flex:1,color:"#8ab4f8",fontSize:12,wordBreak:"break-all",background:"rgba(138,180,248,.08)",padding:"7px 12px",borderRadius:8,fontFamily:"monospace" }}>{localStorage.getItem("currentLiveViewerLink")||"Aucun lien disponible"}</span>
        ...
        <div style={{ flex:1,display:"grid",gap:8,alignContent:"center",overflow:"hidden",minWidth:0,gridTemplateColumns:`repeat(${cols},1fr)` }}>
            ...
            : (
              <div style={{ position:"relative",background:"#111",borderRadius:14,overflow:"hidden",aspectRatio:"16/9",minHeight:90 }}>
                <div style={{ position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:10,background:"radial-gradient(circle at 35% 30%,#1a1a2e,#0d0d1a)" }}>
                  ...
                  <span style={{ color:"#9aa0a6",fontSize:11,background:"rgba(0,0,0,.5)",padding:"4px 12px",borderRadius:20 }}>{micOn?"🎤 Micro actif":"🔇 Micro coupé"}</span>
                </div>
          ...
          {myRole==="guest"&&visiblePeers.length===0&&(
            <div style={{ display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:14,minHeight:300,background:"rgba(255,255,255,.02)",borderRadius:14,border:"1px dashed rgba(255,255,255,.1)" }}>
              ...
              <p style={{ color:"#9aa0a6",fontSize:14 }}>En attente de l'admin…</p>
              <p style={{ color:"#5f6368",fontSize:12 }}>Le flux vidéo apparaîtra quand l'admin active sa caméra</p>
            </div>
          )}
        </div>
                ...
                <div key={p.socketId} style={{ display:"flex",alignItems:"center",gap:8,padding:"8px 4px",borderBottom:"1px solid rgba(255,255,255,.04)" }}>
                  ...
                  {myRole==="host"&&p.socketId!==sockRef.current?.id&&(
                    <div style={{ display:"flex",gap:3 }}>
                      ...
                      <button title="Bloquer" onClick={()=>adminBlock(p.socketId)} style={{ background:"rgba(234,67,53,.2)",border:"1px solid rgba(234,67,53,.4)",borderRadius:6,color:"#ea4335",padding:"4px 7px",cursor:"pointer",fontSize:11,fontWeight:700 }}>🚫</button>
                    </div>
                  )}
                </div>
              ))}
            </div>
            {myRole==="host"&&<div style={{ padding:10,borderTop:"1px solid rgba(255,255,255,.06)" }}><button onClick={muteAll} style={{ background:"rgba(234,67,53,.1)",border:"1px solid rgba(234,67,53,.25)",borderRadius:8,color:"#ea4335",padding:"8px 12px",cursor:"pointer",fontWeight:600,fontSize:12,width:"100%" }}>🔇 Couper tous les micros</button></div>}
          </div>
        )}
        ...
        {aiOpen&&(
          <div style={{ width:320,background:"#2d2f31",borderRadius:14,border:"1px solid rgba(255,255,255,.06)",display:"flex",flexDirection:"column",overflow:"hidden",flexShrink:0,animation:"slideIn .25s ease" }}>
            ...
            <div style={{ flex:1,overflowY:"auto",padding:14 }}>
              {aiLoading?(<div style={{ textAlign:"center",padding:"40px 20px" }}><div style={{ width:44,height:44,border:"3px solid rgba(255,255,255,.1)",borderTopColor:"#8ab4f8",borderRadius:"50%",animation:"spin .8s linear infinite",margin:"0 auto 16px" }} /><p style={{ color:"#9aa0a6",fontSize:13 }}>Analyse en cours…</p></div>)
              :aiText?(<div><div style={{ color:"#e8eaed",fontSize:13,lineHeight:1.8,whiteSpace:"pre-wrap" }}>{aiText}</div><button onClick={()=>{navigator.clipboard.writeText(aiText);showToast("Copié !","#34a853","📋");}} style={{ background:"#1a73e8",color:"#fff",border:"none",padding:"10px 16px",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:12,width:"100%",marginTop:12 }}>📋 Copier</button></div>)
              :(<div style={{ textAlign:"center",padding:"20px 10px" }}><div style={{ fontSize:52,marginBottom:14 }}>🤖</div><p style={{ color:"#9aa0a6",fontSize:13,marginBottom:20,lineHeight:1.7 }}>L'IA générera un résumé complet du live.</p><button onClick={()=>setEndModal(true)} style={{ background:"linear-gradient(135deg,#1a73e8,#0d47a1)",color:"#fff",border:"none",padding:"12px 20px",borderRadius:12,cursor:"pointer",fontWeight:700,fontSize:13,width:"100%" }}>✨ Terminer + Résumé</button></div>)}
            </div>
          </div>
        )}
      </div>
