...
export default function MeetRoom() {
    ...
    (async()=>{
      ...
      try {
        if (myRole==="host") {
          ...
          // Jeune: mic only — muted by default
          const s = await navigator.mediaDevices.getUserMedia({video:false,audio:true});
          if (!alive){ s.getTracks().forEach(t=>t.stop()); return; }
          s.getAudioTracks().forEach(t=>{ t.enabled=false; }); // muted
          localStr.current = s;
        }
      } catch {
  ...
  const copyLink=async()=>{
    if(!viewerLink){ showToast("Aucun lien viewer disponible","#ea4335","❌"); return; }
    await navigator.clipboard.writeText(viewerLink).catch(()=>{});
  ...
  /* ─── Grid layout ─── */
  // ✅ Jeune sees ONLY admin (host) tiles — not other jeunes
