...
async function translate(text, lang) {
  ...
  const res = await fetch(ANTHROPIC_URL, {
    ...
    body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 300,
      messages: [{ role: "user", content: `Traduis en ${map[lang] || lang}. UNIQUEMENT la traduction:\n"${text}"` }] })
  });
  return (await res.json()).content?.[0]?.text?.trim() || text;
}

/* ══════════════════════════════════════════
   VIDEO TILE — works for both host and guest
══════════════════════════════════════════ */
    ...
    <div style={{
      ...
      {!showCam && (
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center",
          justifyContent: "center", background: "radial-gradient(circle at 35% 30%,#222,#0d0d0d)" }}>
          <div style={{
            width: 72, height: 72, borderRadius: "50%", fontSize: 26, fontWeight: 800,
      ...
      {/* Badges */}
      <div style={{ position: "absolute", bottom: 8, left: 8, display: "flex", gap: 5, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {camOff && <span style={{ background: "rgba(234,67,53,.85)", color: "#fff", fontSize: 9, padding: "2px 6px", borderRadius: 6 }}>📷 Off</span>}
      </div>
...
export default function MeetRoom() {
  ...
  const [micOn,         setMicOn]        = useState(myRole === "host");  // host: on, guest: off
  const [camOn,         setCamOn]        = useState(true);               // les deux peuvent avoir cam
  ...
  /* ── viewerLink: d'abord depuis localStorage, sinon reconstruire ──
     ✅ FIX: le lien doit pointer vers /live/:roomCode?vt=... (pas /meet/)
     pour que les gens sans compte voient la page de redirection propre */
  const viewerLink = (() => {
    const stored = localStorage.getItem("currentLiveViewerLink") || "";
    if (stored && stored.includes(roomCode)) return stored;
    // Fallback: reconstruire depuis l'URL courante si on est en mode host
    if (myRole === "host" && vtToken) return `${window.location.origin}/live/${roomCode}?vt=${vtToken}`;
    return stored;
  })();
  ...
  const createPeer = useCallback((sid) => {
    ...
    // ✅ Ajouter les tracks locaux (audio + vidéo si dispo)
  ...
  /* ─── INIT ─── */
  useEffect(() => {
    if (!token) { setErrMsg("Token d'accès manquant. Utilisez le lien fourni par l'admin."); setStatus("error"); return; }
    let alive = true;
    ...
    (async () => {
      ...
      try {
        if (myRole === "host") {
          ...
          try {
            const s = await navigator.mediaDevices.getUserMedia({ video: true, audio: true });
            if (!alive) { s.getTracks().forEach(t => t.stop()); return; }
            // Micro coupé par défaut pour jeune
            ...
            // Si pas de caméra, essayer micro seulement
            try {
      ...
      sock.on("force-kicked", () => { showToast("Vous avez été retiré du live.", "#ea4335", "🚪"); setTimeout(() => { cleanup(); navigate(-1); }, 2000); });
      sock.on("live-ended",   () => { showToast("Le live est terminé.", "#5f6368"); setTimeout(() => { cleanup(); navigate(-1); }, 2000); });
  ...
  /* ✅ Micro — marche pour TOUT LE MONDE */
  const toggleMic = () => {
  ...
  /* ✅ Caméra — marche pour TOUT LE MONDE */
  const toggleCam = () => {
  ...
  /* ✅ Copier lien — admin ET jeune peuvent copier */
  const copyInviteLink = async () => {
    const link = viewerLink || `${window.location.origin}/live/${roomCode}?vt=${vtToken || ""}`;
    if (!link || link.endsWith("vt=")) { showToast("Lien non disponible", "#ea4335", "❌"); return; }
    await navigator.clipboard.writeText(link).catch(() => {});
    setCopied(true); setTimeout(() => setCopied(false), 2500);
    showToast("✅ Lien copié ! Partagez-le pour rejoindre", "#34a853");
  };

  /* ✅ Jeune copie son propre lien de salle */
  const copyMyLink = async () => {
    const link = `${window.location.origin}/live/${roomCode}?vt=${vtToken || ""}`;
    await navigator.clipboard.writeText(link).catch(() => {});
  ...
  /* ── Grid layout ──
     ...
     ✅ Jeune voit l'admin EN GRAND + les autres jeunes en petit */
  ...
  // Pour jeune: admin en principal + autres guests en grille
  // Pour host: tout le monde en grille
  const mainPeer    = myRole === "guest" ? hostPeer : null;
  const gridPeers   = myRole === "host"  ? peers : guestPeers;
  const gridCount   = 1 + gridPeers.length;
    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "#202124", fontFamily: "var(--font)", color: "#fff", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 20px", background: "#2d2f31", flexShrink: 0, borderBottom: "1px solid rgba(255,255,255,.06)" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {/* Compteur participants — visible pour admin */}
          {myRole === "host" && (
            <span style={{ background: "rgba(255,255,255,.07)", color: "#e8eaed", padding: "5px 12px", borderRadius: 20, fontSize: 13, fontWeight: 600 }}>
              👥 {ptcps.length} participant{ptcps.length > 1 ? "s" : ""}
            </span>
          )}

          {/* ✅ Admin: bouton Inviter avec lien viewer correct */}
          {myRole === "host" && (
          ...
          {/* ✅ Jeune: bouton copier son lien pour le partager */}
          {myRole === "guest" && (
            <button className="cbtn" onClick={copyMyLink} style={{ background: "rgba(255,255,255,.06)", border: "1px solid rgba(255,255,255,.1)", color: "#e8eaed", padding: "6px 14px", borderRadius: 8, fontSize: 12, fontWeight: 500 }}>
              {linkCopied ? "✅ Copié !" : "🔗 Partager le lien"}
            </button>
          )}
        </div>
      </div>

      {/* ═══ INVITE LINK BAR (host) ═══ */}
        ...
        <div style={{ background: "#2d2f31", borderBottom: "1px solid rgba(255,255,255,.06)", padding: "10px 18px", flexShrink: 0, animation: "fadeIn .2s ease" }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <span style={{ flex: 1, color: "#8ab4f8", fontSize: 11, wordBreak: "break-all", background: "rgba(138,180,248,.08)", padding: "7px 12px", borderRadius: 8, fontFamily: "monospace" }}>
              {viewerLink || "Créez d'abord un live depuis Nouvelle Réunion"}
            </span>
            {viewerLink && (
              <button onClick={copyInviteLink} style={{ background: "#1a73e8", border: "none", borderRadius: 8, padding: "7px 16px", color: "#fff", cursor: "pointer", fontSize: 12, fontWeight: 700, whiteSpace: "nowrap", flexShrink: 0 }}>
                {copied ? "✅ Copié !" : "📋 Copier"}
              </button>
            )}
          ...
          <p style={{ color: "#5f6368", fontSize: 10, marginTop: 5 }}>Ce lien contient le token d'accès. Les jeunes inscrits pourront rejoindre directement.</p>
        </div>
      )}
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden", gap: 8, padding: 8, minHeight: 0 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8, overflow: "hidden", minWidth: 0 }}>
          ...
          {myRole === "guest" && mainPeer ? (
            /* ✅ JEUNE: Admin en grand écran + grille des autres en bas */
            <>
              <div style={{ flex: 1, overflow: "hidden", borderRadius: 14, background: "#000" }}>
                <Tile
                  stream={mainPeer.stream}
                  name={nameMap.current[mainPeer.id] || "Admin"}
                  role="host"
              ...
              {/* Mes tiles + autres jeunes */}
              {(gridPeers.length > 0 || true) && (
                <div style={{ display: "flex", gap: 8, height: 130, flexShrink: 0, overflowX: "auto" }}>
                  {/* Ma propre caméra */}
                  ...
                  {gridPeers.map(p => (
                    <div key={p.id} style={{ width: 200, flexShrink: 0 }}>
                      <Tile
                        stream={p.stream}
                        name={nameMap.current[p.id] || "Jeune"}
            ...
            /* ✅ ADMIN (ou jeune sans admin): grille de tout le monde */
            <div style={{ flex: 1, display: "grid", gap: 8, alignContent: "center", overflow: "hidden", minWidth: 0, gridTemplateColumns: `repeat(${cols},1fr)` }}>
              {/* Ma tile locale */}
              <Tile localRef={localVid} isLocal muted name={myName} role={myRole} camOff={!camOn} />
              ...
              {msgs.map(m => {
                ...
                return (
                  <div key={m.id} style={{ alignSelf: isMe ? "flex-end" : "flex-start", maxWidth: "86%", animation: "fadeIn .2s ease" }}>
                    ...
                    {translations[m.id] && (
                      <div style={{ padding: "6px 10px", borderRadius: 10, fontSize: 12, marginTop: 3, wordBreak: "break-word", background: "rgba(52,168,83,.12)", border: "1px solid rgba(52,168,83,.2)", color: "#81c995" }}>
                        🌍 {translations[m.id]}
                      </div>
                    )}
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 8 }}>
              {ptcps.map(p => (
                <div key={p.socketId} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 4px", borderBottom: "1px solid rgba(255,255,255,.04)" }}>
                  ...
                  {/* ✅ Admin: boutons mute/kick pour chaque participant (sauf soi-même) */}
                  {myRole === "host" && p.socketId !== sockRef.current?.id && (
                    <div style={{ display: "flex", gap: 3 }}>
                      ...
                      <button title="Couper caméra" onClick={() => adminMute(p.socketId, "video")}
                        style={{ background: "rgba(255,255,255,.06)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 6, color: "#fbbc04", padding: "4px 7px", cursor: "pointer", fontSize: 12 }}>📷</button>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
              {aiLoading ? (
                ...
                <div style={{ textAlign: "center", padding: "20px 10px" }}>
                  ...
                  <p style={{ color: "#9aa0a6", fontSize: 13, marginBottom: 20, lineHeight: 1.7 }}>L'IA analysera le chat et la transcription pour générer une conclusion professionnelle.</p>
                  <button onClick={genConclusion} style={{ background: "linear-gradient(135deg,#1a73e8,#0d47a1)", color: "#fff", border: "none", padding: "12px 20px", borderRadius: 12, cursor: "pointer", fontWeight: 700, fontSize: 13, width: "100%" }}>✨ Générer</button>
                </div>
              )}
            </div>
          </div>
      ...
      <div style={{ background: "#2d2f31", padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, flexShrink: 0, flexWrap: "wrap", borderTop: "1px solid rgba(255,255,255,.05)" }}>
        ...
        {/* Gauche: micro + caméra (TOUT LE MONDE) */}
        <div style={{ display: "flex", gap: 5 }}>
          <Btn icon={micOn ? "🎤" : "🔇"} label={micOn ? "Micro" : "Muet"} onClick={toggleMic} active={micOn} />
          {/* ✅ Caméra pour ADMIN et JEUNE */}
          <Btn icon={camOn ? "📷" : "🚫"} label={camOn ? "Caméra" : "Cam Off"} onClick={toggleCam} active={camOn} />
        </div>
        ...
        <div style={{ display: "flex", gap: 5 }}>
          ...
          <Btn icon="👥" label={`Membres${ptcps.length > 0 ? ` (${ptcps.length})` : ""}`} onClick={() => { setPartOpen(o => !o); setChatOpen(false); setAiOpen(false); }} active />
        </div>
