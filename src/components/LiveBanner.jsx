// LiveBanner.jsx — Bandeau Live partagé entre JeuneLayout, Accueil, Swafy
...
export default function LiveBanner({ compact = false }) {
  ...
  const joinLive = () => {
    ...
    try {
      ...
      // stream_link = https://domain/live/{roomCode}?vt=TOKEN
      const pathParts = url.pathname.split("/").filter(Boolean);
      const roomCode  = pathParts[pathParts.length - 1]; // dernier segment
      const vt        = url.searchParams.get("vt");

      if (roomCode && vt) {
        // ✅ CORRECT: jeune va dans /meet/{roomCode}?vt={token}
        navigate(`/meet/${roomCode}?vt=${vt}`);
      } else if (live.room_code) {
        // Fallback: on a au moins le roomCode en DB
        // Mais on n'a pas de vt → afficher erreur
        alert("Lien invalide. Contactez l'organisateur.");
      } else {
        navigate("/meet");
      }
    } catch {
      // stream_link n'est pas une URL valide
      if (live.room_code) {
