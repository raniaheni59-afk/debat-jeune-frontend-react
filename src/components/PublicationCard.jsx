...
const avatar = (name) => `https://ui-avatars.com/api/?name=${encodeURIComponent(name||"U")}&background=5a3fa0&color=fff&size=80`;
const timeAgo = (d) => {
  if (!d) return "";
...
const ReactionSummary = ({ counts={} }) => {
  const top = REACTIONS.filter(r=>(counts[r.key]||0)>0).sort((a,b)=>(counts[b.key]||0)-(counts[a.key]||0)).slice(0,3);
  const total = Object.values(counts).reduce((s,v)=>s+v,0);
  if (!total) return null;
  return <span className="pc-rs">{top.map(r=><span key={r.key} className="pc-rs-e">{r.emoji}</span>)}<span className="pc-rs-n">{total}</span></span>;
};
    ...
    <div className="pc-stk-picker" ref={ref}>
      ...
      <div className="pc-stk-grid">
        {STICKERS[cat].items.map((s,i) => (
          <button key={i} className="pc-stk-btn" onClick={() => { onPick(s); onClose(); }} type="button">{s}</button>
        ))}
      </div>
    </div>
  );
...
const Comment = ({ comment, pubId, onRefresh, depth=0 }) => {
  ...
  const sendReply = async (txt) => {
    ...
    try {
      ...
      onRefresh();
    } catch(e) { console.error("reply err:",e?.response?.data||e.message); }
  ...
  return (
    <div className={`pc-cmt${depth>0?" nested":""}`}>
      ...
      <img className="pc-cmt-ava"
        src={getImg(comment.photo_user)||avatar(comment.prenom_user)}
      ...
      <div className="pc-cmt-right">
        ...
        <div className="pc-cmt-meta">
          ...
          <div className="pc-cmt-rpick-wrap" ref={picRef}>
            <button className={`pc-cmt-act${myReaction?" on":""}`} onClick={()=>setPickerOpen(o=>!o)} type="button">
          ...
          <div className="pc-reply-row">
            <div className="pc-reply-box">
              <input className="pc-reply-inp"
                ...
                onKeyDown={e=>e.key==="Enter"&&!e.shiftKey&&sendReply()} autoFocus/>
            ...
            {stkOpen && <StickerPicker onPick={s=>setReplyText(t=>t+s)} onClose={()=>setStkOpen(false)}/>}
          </div>
        )}
...
export default function PublicationCard({ publication, onUpdate, defaultShowComments=false }) {
  ...
  useEffect(() => {
    const h = (e) => {
      if (picRef.current && !picRef.current.contains(e.target)) setPickerOpen(false);
    };
    document.addEventListener("mousedown", h);
    return () => document.removeEventListener("mousedown", h);
  }, []);
  ...
  const reactPub = async (key) => {
    ...
    setMyReaction(same?null:key);
    setCounts(c=>{ const n={...c}; if(prev) n[prev]=Math.max(0,(n[prev]||1)-1); if(!same) n[key]=(n[key]||0)+1; return n; });
    try { await API.post(`/publications/${pub.id_publication}/react`,{type_reaction:same?null:key}); onUpdate?.(); }
    catch { onUpdate?.(); }
  };
    ...
    <>
      <article className="pc-card">
        ...
        <div className="pc-header">
          <img className="pc-ava"
            src={getImg(pub.photo_user)||avatar(pub.prenom_user||"U")}
            alt={pub.prenom_user}
        ...
        {body && (
          <p className="pc-body">
            ...
            {isLong && <button className="pc-more-txt" onClick={()=>setExpanded(o=>!o)} type="button">{expanded?"Voir moins":"Voir plus"}</button>}
          </p>
        )}
        ...
        <div className="pc-stats">
          <ReactionSummary counts={counts}/>
          <span className="pc-cmt-cnt" onClick={toggleCmts}>{totalCmt} commentaire{totalCmt!==1?"s":""}</span>
        </div>

        {/* actions */}
        <div className="pc-actions">
          <div className="pc-act-wrap" ref={picRef}>
            <button className={`pc-act-btn${myReaction?" on":""}`}
              onClick={()=>myReaction?reactPub(myReaction):setPickerOpen(o=>!o)} type="button">
              {myDef?<span className="pc-act-emoji">{myDef.emoji}</span>:<ThumbIcon/>}
              <span style={{color:myReaction?"#5a3fa0":"inherit"}}>{myDef?myDef.label:"J'aime"}</span>
            </button>
            {pickerOpen&&<ReactionPicker onPick={reactPub} current={myReaction}/>}
          </div>
          <button className="pc-act-btn" onClick={toggleCmts} type="button"><CommentIcon/><span>Commenter</span></button>
          <button className="pc-act-btn" onClick={()=>navigator.clipboard?.writeText(`${window.location.origin}/publication/${pub.id_publication}`)} type="button">
          ...
          <div className="pc-cmts-section">
              ...
              <div className="pc-cmt-input-wrap">
                ...
                <div className="pc-cmt-inp-acts">
                  <div className="pc-stk-wrap" ref={stkRef}>
                    <button className="pc-cmt-emoji-btn" type="button" onClick={()=>setStkOpen(o=>!o)} title="Stickers">😊</button>
                    {stkOpen&&(
                      <StickerPicker
                  ...
                  <button className="pc-send-btn" onClick={()=>sendComment()} disabled={!cmtText.trim()||cmtSending} type="button">
            ...
            {cmtLoading
              ...
              : comments.length===0
                ...
                : <div className="pc-cmt-list">{comments.map(c=><Comment key={c.id_commentaire} comment={c} pubId={pub.id_publication} onRefresh={loadComments}/>)}</div>}
          </div>
        )}
      </article>
      ...
      {lightbox!==null&&(
        <div className="pc-lightbox" onClick={()=>setLightbox(null)}>
          ...
          {lightbox>0&&<button className="pc-lb-nav left" onClick={e=>{e.stopPropagation();setLightbox(i=>i-1);}}>‹</button>}
          <img className="pc-lb-img" src={getImg(media[lightbox]?.url_media||media[lightbox]?.chemin_fichier)} alt="" onClick={e=>e.stopPropagation()}/>
          {lightbox<media.length-1&&<button className="pc-lb-nav right" onClick={e=>{e.stopPropagation();setLightbox(i=>i+1);}}>›</button>}
        </div>
      )}
    </>
  );
